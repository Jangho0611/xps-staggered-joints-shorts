...
import {DURATIONS, AUDIO_FILES, FOAM_START, CONCLUSION_FRAME} from './timing-v3';
...
const frontStrokes:Stroke[]=[
 {x:195,y:0,dx:0,dy:960,start:171,end:208},
 {x:585,y:0,dx:0,dy:960,start:214,end:251},
 {x:0,y:240,dx:780,dy:0,start:257,end:287},
 {x:0,y:720,dx:780,dy:0,start:293,end:323},
];
...
const Wall=({scene,frame,duration}:{scene:number;frame:number;duration:number})=>{
 ...
 const guide=scene===2?ease(frame,130,140)*(1-ease(frame,162,170)):scene===3?point2*(1-finish):scene===4?ease(frame,12,25)*(1-ease(frame,90,110)):0;
 const id=`wall-${scene}`;
...
const Scene=({index}:{index:number})=>{
 ...
 const secondLine=index===1?ease(frame,FOAM_START-5,FOAM_START+9):index===3?ease(frame,duration*.37,duration*.44):1;
 ...
 let note=index===0?'벽체에 XPS 판재를 시공한 상태':index===1?'노즐이 판재 사이의 좁은 틈새를 따라 이동':index===2?(frame<171?'점선: 1층 이음부 · 실선: 2층 이음부':'2층 판재 사이의 틈새도 충진'):index===3?(final?'틈새 처리와 엇갈림을 함께':'2층 일부를 열어 본 설명용 투시'):frame<110?'점선: 1층 이음부 · 실선: 2층 이음부':'틈새 처리 + 이음부 엇갈림';
...
const Root=()=> <><Composition id="XpsStaggeredV3" component={Film} width={1080} height={1920} fps={30} durationInFrames={DURATIONS.reduce((a,b)=>a+b,0)+170}/>{DURATIONS.map((n,i)=><Composition key={i} id={`Scene0${i+1}V3`} component={Scene} defaultProps={{index:i}} width={1080} height={1920} fps={30} durationInFrames={n}/>)}<Composition id="EndingV3" component={Ending} width={1080} height={1920} fps={30} durationInFrames={170}/></>;
registerRoot(Root);
