...
const Scene=({index}:{index:number})=>{
 ...
 const scene4Second=index===3&&frame>=Math.round(duration*0.48);
 const lines=index===0?['두꺼운 XPS','한 장이면 끝?']:index===1?['판재 사이 틈새','우레탄폼으로 충진']:index===2?['2P 시공의 핵심','이음부를 엇갈리게']:index===3?(scene4Second?['2P','이음부를 엇갈리게']:['1P','이음부가 그대로']):['두께보다 중요한 건','이음부 마감 디테일'];
...
const Root=()=> <><Composition id="XpsStaggeredV6" component={Film} width={1080} height={1920} fps={30} durationInFrames={DURATIONS.reduce((a,b)=>a+b,0)+170}/>{DURATIONS.map((n,i)=><Composition key={i} id={`Scene0${i+1}V6`} component={Scene} defaultProps={{index:i}} width={1080} height={1920} fps={30} durationInFrames={n}/>)}<Composition id="EndingV6" component={Ending} width={1080} height={1920} fps={30} durationInFrames={170}/></>;
registerRoot(Root);
