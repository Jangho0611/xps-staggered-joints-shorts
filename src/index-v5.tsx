...
import {DURATIONS, AUDIO_FILES, FOAM_START, CONCLUSION_FRAME} from './timing-v4';
...
const Scene=({index}:{index:number})=>{
 ...
 const lines=index===0?['두꺼운 XPS','한 장이면 끝?']:index===1?['판재 사이 틈새','우레탄폼으로 충진']:index===2?['2P 시공의 핵심','이음부를 엇갈리게']:index===3?(final?['2P는 서로 엇갈리게']:['1P는 이음부가 이어지고','2P는 서로 엇갈리게']):['두께만 늘리는 게 아니라','이음부를 엇갈려 시공'];
...
const Root=()=> <><Composition id="XpsStaggeredV5" component={Film} width={1080} height={1920} fps={30} durationInFrames={DURATIONS.reduce((a,b)=>a+b,0)+170}/>{DURATIONS.map((n,i)=><Composition key={i} id={`Scene0${i+1}V5`} component={Scene} defaultProps={{index:i}} width={1080} height={1920} fps={30} durationInFrames={n}/>)}<Composition id="EndingV5" component={Ending} width={1080} height={1920} fps={30} durationInFrames={170}/></>;
registerRoot(Root);
