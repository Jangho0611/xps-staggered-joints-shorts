...
rows=json.loads((p/'docs/timing-v3.json').read_text())
jobs=[(f'Scene{i:02}V3',f'scene{i:02}-preview-candidate-v3.mp4',rows[i-1]['frames']) for i in range(1,6)]+[('XpsStaggeredV3','xps-staggered-joints-preview-v3.mp4',sum(r['frames'] for r in rows)+170)]
qa=[]
for comp,name,expected in jobs:
 out=p/'public/assets/video'/name
 assert not out.exists(),f'Refusing overwrite: {out}'
 with (p/f'out/qa/{comp}-render-v3.log').open('x') as log:
  subprocess.run(['npx','remotion','render','src/index-v3.tsx',comp,str(out),'--codec=h264','--audio-codec=aac','--concurrency=2'],cwd=p,stdout=log,stderr=subprocess.STDOUT,check=True)
...
subprocess.run(['ffmpeg','-v','error','-n','-i',str(p/'public/assets/video/xps-staggered-joints-preview-v3.mp4'),'-vf','fps=1/2,scale=216:384,tile=6x4','-frames:v','1',str(p/'out/qa/contact-v3.jpg')],check=True)
checks=json.loads((p/'docs/preservation-before-v3.json').read_text())
for rel,h in checks.items():assert hashlib.sha256((p/rel).read_bytes()).hexdigest()==h,rel
(p/'out/qa/mechanical-v3.json').write_text(json.dumps({'videos':qa,'v1V2AndEndingPreserved':True,'checkedFiles':len(checks)},indent=2))
